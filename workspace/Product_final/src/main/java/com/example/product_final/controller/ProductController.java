...
@Controller
@Slf4j
// RequestMapping 은 FrontController 를 하는 역활을 수행한다.
...
//        return "product/index.html";
        // 기본 시작 경로는 templates
        // 우리 인덱스 페이지는 product 안에 있기에 경로를 명시해야한다!
        // 그 뒤에 .html 확장자는 생략이 가능하다!
        return "product/index";
    }

    @GetMapping("/list")
    // Model 은 컨트롤러에서 html 로 데이터를 전달하기 위해 사용하는 객체이다.
    // 해당 객체에 데이터를 저장, 그리고 html 로 가서 타임리프를 뿌려줌.
    public String list(Model model) {

        // 앞에 적어준 문자열은 html 내에서 사용할 데이터의 이름
        // 뒤에 있는 문자열을 실제로 전송하는 데이터.
        model.addAttribute("products", productService.findAll());

        // th:if 를 사용 하기전에 하나만 넘겨보기
//        model.addAttribute("product", productService.findAll().get(0));
    ...
    //    @PathVariable
//    경로로 넘어온 값을 매개변수와 매핑시킬때 사용한다.
//    PathVariable 의 이름과 매개변수의 이름이 동일하다면 자동으로 매핑되기 때문에 생략이 가능하다.
    ...
    public String writeOk(@ModelAttribute ProductDTO2 product, RedirectAttributes redirectAttributes) {
        ...
        //리스트를 보여주고, html 을 하나 띄워줄건데
        // 그 html 의 내용은 게시글이 정상적으로 추가되었습니다.

        int isInserted = productService.save(vo);

        // 게시글에 목록을 추가하면 '추가 되었습니다' 라는 메시지 출력하는 코드
        if (isInserted != 1) {
    ...
    public String delete(@PathVariable Long id, RedirectAttributes redirectAttributes){
        ...
        // 목록을 삭제 했을때 '목록이 삭제 되었습니다' 라는 메시지를 출력하는 코드
        if(isDeleted != 1){
            redirectAttributes.addFlashAttribute("error", "예상치 못한 에러가 발생했습니다.");
        } else {
